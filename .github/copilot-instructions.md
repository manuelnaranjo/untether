...
Untether is a Telegram bridge for AI coding agents (Claude Code, Codex, OpenCode, Pi, Antigravity CLI, Amp). It runs on the user's machine and bridges agents to Telegram, so users can send tasks by voice or text, approve changes, and read results from their phone — while agents work in the background.
